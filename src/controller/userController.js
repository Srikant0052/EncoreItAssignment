...
const createUser = async (req, res) => {
    try {
        ...
        const { name, age, email, password, roles } = requestBody;
        if (!isValid(name)) {
        ...
        if (['Admin', 'User', 'Guest'].indexOf(roles) == -1) {
            return res.status(400).send({ status: false, msg: "User role will be Admin, User or Guest" })
        }
        const userData = {
            name: name, age: age, email: email, password: hashPassword, roles: roles
        }
...
module.exports = { createUser, userLogin, userList };